...
int ThermalCamera::getFrame(ThermalFrame &outFrame)
{
    ...
    case 0: // Subpage 0 update old pixels
        for (int col = 0; col < FRAME_COLS; col ++){
            ...
            for (int row = 0; row < FRAME_ROWS; row++){
                ...
                if(row % 2 == 0 && col % 2 != 0){
                    // IF even row then odd cols are old and need to be updated 
                    outFrame.pixels[pixIndex] = externFramePtr->pixels[pixIndex];
                }else if (row % 2 != 0 && col % 2 == 0) {
                    // If odd row then even pixels need to be updated 
                    outFrame.pixels[pixIndex] = externFramePtr->pixels[pixIndex];
                }
            }
        }
    ...
    case 1: // Subpage 1 update old pixels 
        for (int col = 0; col < FRAME_COLS; col ++){
...
int ThermalCamera::setRefreshRate(TC_RefreshRate rate)
{
    int status = MLX90640_SetRefreshRate(MLX906040_I2C_ADRR, static_cast<uint8_t>(rate));

    if (status != 0){
        LOG_ERR("setRefreshRate failed with status: %d",status);
    }
